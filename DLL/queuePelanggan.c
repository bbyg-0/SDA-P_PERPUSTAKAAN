...
void insertPelanggan (addressBuku *head, addressPelanggan *history, addressBuku Buku, char * input1, char input2){
	...
	if(Pelanggan == Pelanggan->next) printf("DI insertPelanggan");
	if(Buku->Stok > 1){
		Buku->Stok--;
		...
		movNode(&((*head)->data->peminjam), &Pelanggan, (Pelanggan)->Nama, (Pelanggan)->Prioritas, NULL);
		return;
	}
	...
	tambahNote(&Pelanggan, input1);
	if(isEmpty((*head)->data->Q)){
		(Pelanggan)->next = NULL;
		(*head)->data->Q = (Pelanggan);
		return;
	}
	addressPelanggan temp = (*head)->data->Q;
	addressPelanggan temp2 = (*head)->data->Q;
...
void batalkanPelanggan (addressBuku *head, addressPelanggan *history, char * input1, char input2){
	...
	movNode(&Pelanggan, &((*head)->data->Q), input1, input2, NULL);
	if(isEmpty(Pelanggan)){printf("KOSONG BRADA!!"); return;}
	if(Pelanggan == Pelanggan->next) printf("DI batalkanPelanggan");
		
	for(int i = 0; i < (int)sizeof(input1); i++) input1[i] = '\0';
					
	strcat(input1, "Membatalkan pengajuan peminjaman buku:\n\t");
	strcat(input1, (*head)->Judul);
	strcat(input1, "\n");
...
void pelangganMengembalikan (addressBuku *head, addressPelanggan *history, char * input1, char input2){
	...
	movNode(&Pelanggan, &((*head)->data->peminjam), input1, input2, NULL);
...
void prosesPelanggan (addressBuku *head){
	...
	if((*head)->Stok <= 0){printf("\nSTOK BUKU KOSONG, MEMBATALKAN PROSES\n"); return;}
	movNode(&Pelanggan, &((*head)->data->Q), ((*head)->data->Q->Nama), ((*head)->data->Q->Prioritas), NULL);

	printf("MEMPROSES %s [%c]\n", (Pelanggan)->Nama, (Pelanggan)->Prioritas);
	
	(*head)->Stok--;
	strcat(InputS1, "Memproses :\n\t");
	strcat(InputS1, (*head)->Judul);
	strcat(InputS1, "\n");
