...
void Create_memory (void ** node, DataType data){
	switch(data){
		case NRLL:{
			...
			buku->data = (addressData)malloc(sizeof(Data));
			if(isEmpty(buku->data)){
				printf("\nMEMORI GAGAL DIALKOASI\nMENGHAPUS ALOKASI SEBELUMNYA\n");
				free(buku);
				buku = NULL;
				return;
			}
			buku->data->Q = NULL;
			buku->data->peminjam = NULL;
			buku->Stok = 1;
			buku->Judul = NULL;
			buku->next = NULL;
...
void Tampil_List (void * node[], DataType data, void * x){
	switch(data){
		case NRLL:{
			printf("DAFTAR BUKU:\n");
			int i = 0;
			addressBuku temp = (addressBuku)node[i];
			while (!isEmpty((void *)temp)){
				printf("===================\n%s\nStok\t:%u\n", (temp)->Judul, (temp)->Stok);
				i++;
				temp = (addressBuku)node[i];
			}
			break;
		}
		case Q:{
			...
			while(!isEmpty(temp)){
				...
				if(temp == temp->next){printf("YADSAKDYGSFV"); return;}
				temp = temp->next;
			}
			break;
		}
